...
import {
  ...
  DamagedStockModal,
  CrossDockModal
} from './components/HR/InventoryModals';
import {
  ...
  ShiftScheduler,
  OrgChart
} from './components/HR/DepartmentModal';
import {
  BudgetModal,
  CashFlowModal,
  FixedAssetModal,
...
function AIToastOverlay({ user, onNavigate }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Prefer critical/unread operational alerts; include briefings without emoji/AI branding
        const candidates = res.recent.filter(n => !shownIds.has(n.id) && !n.read && (n.isAI || n.priority === 'critical' || n.priority === 'high'));
        if (candidates.length > 0) {
          setToasts(prev => {
            const ids = new Set(prev.map(t => t.id));
            const next = candidates.filter(n => !ids.has(n.id)).slice(0, 3);
            if (next.length) playSound();
  ...
  return (
    <div className="ai-toast-container">
      {toasts.length > 1 && (
        <button className="ai-toast-dismiss-all" type="button" onClick={dismissAll}>Dismiss all ({toasts.length})</button>
      )}
      {toasts.map(n => {
        const title = cleanToastText(n.title);
        const message = cleanToastText(n.message);
        const source = n.sourceLabel && n.sourceLabel !== 'FarmTrack AI' ? n.sourceLabel : (n.category || n.sourceModule || 'Alert');
        return (
          <div key={n.id} className={`ai-toast ${expanded === n.id ? 'expanded' : ''} priority-${n.priority || 'medium'}`} onClick={() => setExpanded(expanded === n.id ? null : n.id)}>
            <div className="ai-toast-header">
              <span className="ai-toast-badge ops">{String(source).slice(0, 18)}</span>
              <span className="ai-toast-time">{timeAgo(n.createdAt)}</span>
              <button type="button" className="ai-toast-close" onClick={e => { e.stopPropagation(); dismiss(n.id); }}>×</button>
            </div>
            <div className="ai-toast-title">{title}</div>
            {(expanded === n.id || message.length < 90) && message && <div className="ai-toast-message">{message}</div>}
            <div className="ai-toast-actions">
              <button type="button" className="ai-toast-action" onClick={e => { e.stopPropagation(); onNavigate('notifications'); dismiss(n.id); }}>Open alerts</button>
              <button type="button" className="ai-toast-action dismiss" onClick={e => { e.stopPropagation(); dismiss(n.id); }}>Dismiss</button>
            </div>
          </div>
        );
      })}
...
function App() {
  ...
  const changeGlobalPeriod = next => {
    const value = next || 'Month';
    setGlobalPeriod(value);
    try { localStorage.setItem('farmtrack-period', value); } catch {}
    // Force every workspace to refetch with the new period
    try { serverCache.clear(); serverInFlight.clear(); } catch {}
    ...
    <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      <main className="main-shell">
        ...
        <div className="content-grid" key={`${page}-${dataVersion}-${globalPeriod}`}>
          {page === 'dashboard' && <Dashboard user={user} setPage={setPage} globalPeriod={globalPeriod} setGlobalPeriod={changeGlobalPeriod} />}
...
function avatarSrc(user) {
  ...
  return `https://ui-avatars.com/api/?name=${name}&background=050505&color=ffffff&size=128&bold=true`;
}

function UserAvatar({ user, size = 36, className = '' }) {
  const [failed, setFailed] = useState(false);
  const src = !failed ? avatarSrc(user) : '';
  if (src) {
    return (
      <img
  ...
  return <div className={`avatar ${className}`} style={{ width: size, height: size }}>{user?.name?.[0] || 'U'}</div>;
}


/** Optimistic data entry: update UI immediately, roll back on failure */
async function optimisticSave(applyLocal, remoteCall) {
  let rollback = null;
  try {
    rollback = typeof applyLocal === 'function' ? applyLocal() : null;
  } catch {}
  try {
...
function Sidebar({ page, setPage, open, setOpen, collapsed, setCollapsed, user, onLogout }) {
    ...
    <>
      <aside className={`sidebar ${open ? 'open' : ''}`}>
        ...
        <div className="sidebar-user-wrap" ref={userMenuRef}>
          <button type="button" className={`sidebar-user ${userMenuOpen ? 'open' : ''}`} onClick={() => setUserMenuOpen(v => !v)} aria-haspopup="menu" aria-expanded={userMenuOpen}>
            <UserAvatar user={user} size={40} />
            <div>
              <strong>{user.name}</strong>
              <span>{user.role}</span>
            </div>
            <ChevronDown size={18} className={userMenuOpen ? 'chev-open' : ''} />
          </button>
          {userMenuOpen && (
            <div className="sidebar-user-menu" role="menu">
              ...
              <button type="button" role="menuitem" className="danger" onClick={() => { setUserMenuOpen(false); onLogout?.(); }}><LogOut size={15} /> Sign out</button>
            </div>
          )}
        </div>
      </aside>
      {open && <button type="button" className="mobile-scrim" onClick={() => setOpen(false)} aria-label="Close menu" />}
    </>
  );
}
...
function Topbar({ user, onMenu, onToggleSidebar, sidebarCollapsed, onNew, onLogout, setPage, page, period, setPeriod }) {
  ...
  // Close bell / sheets / profile menus on outside click
  useEffect(() => {
    if (!bellOpen && !sheetsOpen && !profileOpen) return;
    const close = e => {
      if (bellOpen && !e.target.closest('.notify-dropdown-wrap')) setBellOpen(false);
      if (sheetsOpen && !e.target.closest('.sheets-dropdown-wrap')) setSheetsOpen(false);
      if (profileOpen && !e.target.closest('.topbar-profile-wrap')) setProfileOpen(false);
    };
    document.addEventListener('click', close);
    return () => document.removeEventListener('click', close);
  }, [bellOpen, sheetsOpen, profileOpen]);

  const runSheetsExport = async (moduleName) => {
    setSheetsBusy(true);
    setSheetsMsg('');
    try {
      const res = await rpc('exportSpreadsheetModule', [user, { module: moduleName || page || 'sales', period }]);
      setSheetsMsg(res?.message || res?.status || 'Export prepared');
      if (res?.url) window.open(res.url, '_blank', 'noopener,noreferrer');
      else if (res?.rows) downloadRowsFile(`export-${moduleName || page}-${period}`, res.rows, 'CSV');
    } catch (err) {
      // Fallback: open the live sheet + export a local CSV of search-ish summary
      setSheetsMsg(err.message || 'Opened live sheet');
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      rpc('globalSearch', [user, q, { page: page || '' }])
        .then(rows => {
  ...
  return (
    <header className="topbar">
      ...
      <div className="command-search command-search-wide">
        <Search size={18} />
        <input
          value={query}
      ...
      <div className="topbar-actions">
        ...
        <div className="date-chip topbar-period" title="Change reporting period for all pages">
          <Calendar size={16} />
        ...
        <div className="topbar-profile-wrap">
          ...
          >
            ...
            <span className="topbar-profile-name">{user.name?.split(' ')[0] || 'Account'}</span>
            <ChevronDown size={14} />
          </button>
          {profileOpen && (
            <div className="topbar-profile-menu" role="menu" onClick={e => e.stopPropagation()}>
              ...
              <button type="button" role="menuitem" className="danger" onClick={() => { setProfileOpen(false); onLogout?.(); }}><LogOut size={15} /> Sign out</button>
            </div>
          )}
        </div>
      </div>
...
function Dashboard({ user, setPage, globalPeriod = 'Month', setGlobalPeriod = () => {} }) {
  ...
  const { loading, data, error } = useServer(user, 'getDashboardData', [{ period: globalPeriod }], [globalPeriod]);
  ...
  const categories = Array.isArray(data.charts?.categorySales) ? data.charts.categorySales.slice(0, 5) : [];
...
function AnalyticsCenter({ user, setPage, globalPeriod = 'Month' }) {
  ...
  function compareValue(value, label) {
    ...
    // Prefer real prior-period values from tab payload when available
    const prior = active?.priorPeriod?.[label] ?? active?.priorPeriod?.[String(label).toLowerCase()];
    let compared = prior != null && !isNaN(Number(prior)) ? Number(prior) : null;
    if (compared == null) {
      // Fallback ratios only when prior period is missing — clearly labeled as estimate
      const ratio = { 'Previous Month': 0.92, 'Previous Quarter': 0.85, 'Previous Year': 0.78, 'Company Average': 1.0, 'Department Average': 0.95, 'Target': 1.05 }[compareTo] || 1;
      compared = Number(value) * ratio;
      const diff = ((Number(value) - compared) / Math.max(1, Math.abs(compared))) * 100;
      const sign = diff >= 0 ? '+' : '';
      return `${sign}${diff.toFixed(1)}% vs ${compareTo} (est.)`;
    }
    const diff = ((Number(value) - compared) / Math.max(1, Math.abs(compared))) * 100;
    const sign = diff >= 0 ? '+' : '';
  ...
  return (
    <section className="page-stack analytics-page">
      ...
      <section className="analytics-hero executive-analytics-hero">
        <div>
          ...
          {data.dataSource && (
            <div className={`data-source-badge decision-hero-badge ${data.dataSource.normalized || data.dataSource.materializedViews ? 'live' : 'fallback'}`}>
              ...
              <div>
                <strong>{data.dataSource.mode || 'Supabase materialized views'}</strong>
                <em>{data.dataSource.message || 'Analytics is reading precomputed Supabase analytics views.'}</em>
              </div>
              <small>
                {(data.hero?.dataSources || [
        ...
        <div className="confidence-ring">
          <strong>{data.hero?.confidence ?? 97}%</strong>
          <span>Decision confidence</span>
        </div>
      </section>
      <div className="analytics-exec-strip">
        {executiveActions.map(action => {
          const Icon = action.icon;
          return (
            <button key={action.title} type="button" className="analytics-exec-chip" onClick={() => setPage?.(action.page)}>
              <Icon size={16} />
              <span>
      ...
      <div className="dashboard-grid">
        ...
        <Panel className="span-4" title="Inventory Intelligence" action={<button type="button" onClick={() => setPage('inventory')}>Open inventory</button>}>
          <MetricStack items={[
            ['Healthy', data.inventoryIntelligence?.healthy ?? 0],
            ['Low', data.inventoryIntelligence?.low ?? 0],
            ['Dead', data.inventoryIntelligence?.dead ?? 0],
            ['Fast moving', data.inventoryIntelligence?.fastMoving ?? 0],
            ['Slow moving', data.inventoryIntelligence?.slowMoving ?? 0],
            ['Turnover', `${data.inventoryIntelligence?.turnover ?? 0}x`]
          ]} />
        </Panel>
        ...
        <Panel className="span-6" title="Sales Intelligence" action={<button type="button" onClick={() => setPage('sales')}>Open sales</button>}>
          <div className="funnel">
            {(data.salesIntelligence?.funnel || []).map((stage, index) => (
              <div key={stage.stage || index} style={{ width: `${Math.max(40, 100 - index * 10)}%` }}>
                <span>{stage.stage}</span>
                <strong>{stage.count}</strong>
                <em>{currency(stage.value)}</em>
              </div>
            ))}
            {!(data.salesIntelligence?.funnel || []).length && <div className="empty-state">No funnel stages for this period.</div>}
          </div>
        </Panel>
        <Panel className="span-6" title="Financial Intelligence" action={<button type="button" onClick={() => setPage('finance')}>Open finance</button>}>
          <MetricStack items={[
            ['Cash 30 days', currency(data.financialIntelligence?.cash30 ?? 0)],
            ['Cash 60 days', currency(data.financialIntelligence?.cash60 ?? 0)],
            ['Cash 90 days', currency(data.financialIntelligence?.cash90 ?? 0)],
            ['AR risk items', data.financialIntelligence?.arRisk ?? 0],
            ['Profitability', `${data.financialIntelligence?.profitability ?? 0}%`]
          ]} />
        </Panel>
        <Panel className="span-7" title="AI Intelligence" action={<ExportFormatStrip formats={REPORT_FORMATS} onExport={exportCurrentAnalytics} />}>
          <div className="ai-insights clean-ai-insights">
            <div className="analytics-storyline" style={{ marginBottom: 16 }}>
              <article className="analytics-story-card primary">
                <span>Operating summary</span>
                <strong>{(active?.insights || data.aiIntelligence || []).length} insights</strong>
                <p>
                  {(active?.insights || data.aiIntelligence || []).filter(i => (i.confidence || '').toLowerCase() === 'high').length} high confidence ·{' '}
                  {(active?.insights || data.aiIntelligence || []).filter(i => i.action).length} need action
                </p>
              </article>
            </div>
            {(active?.insights || data.aiIntelligence || []).map(item => (
              <article key={item.question} className="ai-insight-row">
                <div className="ai-insight-head">
                  <strong>{item.question}</strong>
                  <span className={`conf-pill conf-${String(item.confidence || 'medium').toLowerCase()}`}>{item.confidence || 'Medium'}</span>
                </div>
                <p>{item.answer}</p>
                <div className="ai-insight-foot">
                  <span>Sources: {(item.records || []).join(', ') || 'operational modules'}</span>
                  <button type="button" onClick={() => item.actionPage && setPage(item.actionPage)}>{item.action || 'Open module'}</button>
                </div>
              </article>
            ))}
            {!(active?.insights || data.aiIntelligence || []).length && <div className="empty-state">No intelligence notes for this period.</div>}
          </div>
        </Panel>
        <Panel className="span-5" title="Executive priorities">
...
function AnalyticsSourcePanel({ source = {}, hero = {} }) {
  ...
  const live = !!source.normalized || !!source.materializedViews;
  return (
    <section className={`analytics-source-panel decision-badge ${live ? 'live' : 'fallback'}`}>
      <div className="decision-badge-main">
        <span className="decision-kicker">Decision layer</span>
        <strong>{source.mode || 'Supabase materialized views'}</strong>
        <p>{source.message || 'Analytics is reading precomputed Supabase analytics views.'}</p>
        <div className="decision-view-chips">
          {coreViews.map(v => (
            <code key={v}>{v}</code>
          ))}
        </div>
      </div>
      <div className="decision-badge-metrics">
        <article>
          <span>Status</span>
          <strong>{source.status || (live ? 'Live' : 'Fallback')}</strong>
        </article>
        <article>
        ...
        <article>
          <span>Last sync</span>
          <strong>{source.lastSync ? new Date(source.lastSync).toLocaleTimeString() : '—'}</strong>
        </article>
      </div>
    </section>
  );
      ...
      <ResponsiveContainer width="100%" height={32}>
        <AreaChart key={animKey} data={data} margin={{ top: 4, right: 2, left: 2, bottom: 0 }}>
          ...
          <Area
            ...
            dataKey="v"
            stroke={color}
            fill={`url(#spark-${tone || 'green'}-${animKey.slice(0, 12)})`}
            strokeWidth={2.25}
            isAnimationActive
            animationBegin={80}
    ...
    <article className={`panel ${className}`}>
      <header>
        ...
        {action && (typeof action === 'string'
          ? <span className="panel-action-label">{action}</span>
...
function CRMWorkspace({ user, setPage, globalPeriod = 'Month' }) {
  ...
  const customers = allCustomers.filter(c => [c.name, c.email, c.phone, c.city, c.type].join(' ').toLowerCase().includes(query.toLowerCase()));
  const pipelineStages = ['New', 'Contacted', 'Proposal', 'Negotiation', 'Won', 'Lost'];
  const crmAnalytics = {
    stageRows: pipelineStages.map(stage => {
      const rows = allLeads.filter(lead => lead.stage === stage || (stage === 'New' && lead.stage === 'Lead'));
      return { stage, opportunities: rows.length, value: rows.reduce((sum, row) => sum + num(row.value), 0) };
    }),
    followUps: allCalls.filter(row => row.followUpDate || ['To Be Called', 'Pending Calls', 'To Be Meeting'].includes(row.stage)).slice(0, 8),
  ...
  return (
    <section className="page-stack crm-workspace">
      ...
      {view === 'overview' && (
        <>
          <div className="control-grid">
            <KpiCard icon={Users} label="Active Customers" value={overview.activeCustomers || allCustomers.filter(c => c.status === 'Active').length} change={12.5} tone="green" />
            <KpiCard icon={Target} label="Opportunities" value={overview.opportunities || allLeads.length} change={8.2} tone="blue" />
            <KpiCard icon={CheckCircle2} label="Won Deals" value={overview.wonDeals || allLeads.filter(l => l.stage === 'Won').length} change={15.3} tone="green" />
            <KpiCard icon={Phone} label="Calls Logged" value={allCalls.length} change={0} tone="green" />
            <KpiCard icon={LineChart} label="Pipeline Stages" value={pipelineStages.length} change={0} tone="blue" />
            <KpiCard icon={Calendar} label="Follow-ups" value={overview.pendingFollowups || crmAnalytics.followUps.length} change={-4.2} tone="red" />
          </div>
          <div className="dashboard-grid">
            <Panel className="span-4" title="Sales Funnel" action="Live">
              <div className="crm-funnel">
                {(data.funnel || crmAnalytics.stageRows.map(s => ({ stage: s.stage, count: s.opportunities }))).map((stage, index) => (
                  <div key={stage.stage} style={{ '--w': `${100 - index * 11}%` }}>
                    <span>{stage.stage}</span><strong>{stage.count}</strong><em>—</em>
                  </div>
                ))}
              </div>
            </Panel>
...
{view === 'activities' && <Panel title="Activity Timeline"><CRMActivityList activities={data.activities} /></Panel>}
      ...
      {selectedCustomer && (
        <CRMCustomerDetail
          ...
          onLogCall={(c) => setModal({ type: 'call', preset: c })}
          onEmailCustomer={(c) => { const to = prompt(`Send email to ${c.name}:`, c.email || ''); if (to) window.location.href = `mailto:${to}?subject=Following up on your account`; }}
          onFollowSaved={async (payload) => {
            await rpc('saveCall', [user, {
              ...payload,
      ...
      <div className="crm-card-grid">
        ...
        {shown.map(customer => (
          <article key={customer.id} className={customer.health === 'VIP' ? 'vip' : customer.health === 'Prospect' ? 'prospect' : ''} onClick={() => onSelect?.(customer)} role="button" tabIndex={0}>
            ...
            <strong>{customer.name}</strong>
            <em>
              <span className="crm-type-chip">{customer.type || 'Customer'}</span>
              {' · '}{customer.city || 'No county'}
            </em>
...
function CRMCustomerDetail({ customer, orders = [], calls = [], deliveries = [], onClose, overlaySize = 'default', setOverlaySize, onLogCall, onEmailCustomer, onFollowSaved }) {
  const [fu, setFu] = useState({ followUpDate: '', nextStep: '', comments: '', phone: customer.phone || '', stage: 'Follow-up' });
  ...
  const timeline = [
    ...
    ...customerCalls.map(c => ({ type: 'Call', date: c.date || c.createdAt, title: c.stage, detail: [c.notes, c.comments].filter(Boolean).join(' · '), icon: Phone, color: '#22c55e' })),
  ...
  return (
    <div className="retractable-overlay crm-detail-overlay" onClick={onClose}>
      <div className={`modal-card overlay-scrollable crm-customer-detail wide-full ${sizeClass}`} onClick={e => e.stopPropagation()} style={{ width: 'min(96vw, 1200px)', maxHeight: '92vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <Panel title="Quick follow-up on this customer" action="Date · phone · comments">
          <form className="settings-form-grid" onSubmit={async e => {
            e.preventDefault();
            if (!fu.followUpDate) return alert('Pick a follow-up date');
            setFuBusy(true);
            try {
              await onFollowSaved?.({
                customerId: customer.id,
                customerName: customer.name,
                phone: fu.phone || customer.phone,
                followUpDate: fu.followUpDate,
                notes: fu.nextStep,
                comments: fu.comments,
                stage: fu.stage
              });
              setFu({ followUpDate: '', nextStep: '', comments: '', phone: customer.phone || '', stage: 'Follow-up' });
            } catch (err) {
              alert(err.message || 'Could not save follow-up');
            } finally {
              setFuBusy(false);
            }
          }}>
            <label>Phone<input value={fu.phone} onChange={e => setFu({ ...fu, phone: e.target.value })} /></label>
            <label>Follow-up date<input type="date" value={fu.followUpDate} onChange={e => setFu({ ...fu, followUpDate: e.target.value })} required /></label>
            <label>Stage
              <select value={fu.stage} onChange={e => setFu({ ...fu, stage: e.target.value })}>
                {['Follow-up', 'To Be Called', 'To Be Meeting', 'Pending Calls', 'Closed'].map(s => <option key={s}>{s}</option>)}
              </select>
            </label>
            <label>Next step<input value={fu.nextStep} onChange={e => setFu({ ...fu, nextStep: e.target.value })} placeholder="Call, quote, visit..." /></label>
            <label>Comments<textarea value={fu.comments} onChange={e => setFu({ ...fu, comments: e.target.value })} rows={2} placeholder="Customer comments" /></label>
            <button type="submit" className="primary-action" disabled={fuBusy}>{fuBusy ? 'Saving...' : 'Save follow-up'}</button>
          </form>
        </Panel>
        <div className="dashboard-grid">
...
function CRMDeliveryPreview({ user, rows = [], onUpdated, compact = false }) {
  ...
  const statuses = ['Pending Delivery', 'Picked', 'Ready for Dispatch', 'Dispatched', 'In Transit', 'Arrived', 'Delivered', 'Failed', 'Returned'];
  ...
  function actionsFor(row) {
    ...
    return [
      { label: 'Set Destination', icon: <MapPin size={15} />, disabled, onClick: () => promptUpdate(row, 'destination', 'Delivery destination', row.destination) },
      { label: 'Means of delivery', icon: <Truck size={15} />, disabled, onClick: () => {
        const value = window.prompt(`Method (${methods.join(', ')})`, row.deliveryMethod || row.method || 'Company Vehicle');
        if (value === null) return;
        update(row, { deliveryMethod: value });
      } },
      { label: 'Set driver / vehicle', icon: <UserCog size={15} />, disabled, onClick: () => {
        const driver = window.prompt('Driver name', row.driver || '');
        if (driver === null) return;
        const vehicle = window.prompt('Vehicle', row.vehicle || '');
        if (vehicle === null) return;
        update(row, { driver, vehicle });
      } },
      { label: 'Add change note', icon: <FileText size={15} />, disabled, onClick: () => {
        const text = window.prompt('Note about change (saved permanently)', '');
        if (text === null || !text.trim()) return;
        update(row, { addNote: text.trim() });
      } },
      ...statuses.map(st => ({
        label: `Status: ${st}`,
        icon: <CheckCircle2 size={15} />,
        disabled,
        onClick: () => update(row, { status: st })
      })),
      { label: 'Mark Pending', icon: <Clock size={15} />, disabled, onClick: () => update(row, { status: 'Pending Delivery', deliveredConfirmed: false }) },
      { label: 'Confirm Delivered', icon: <CheckCircle2 size={15} />, disabled, onClick: () => update(row, { deliveredConfirmed: true }) },
          ...
          {fields[type].map(field => (
            <label key={field}>{field === 'salesOwner' ? 'Sales owner' : label(field)}
              {field === 'customerId' ? (
                ...
                <input type={inputKind(field)} value={form[field] || ''} onChange={e => setForm({ ...form, [field]: e.target.value })} required={!(type === 'call' && field === 'customerId') && isRequiredInput(field)} placeholder={field === 'salesOwner' ? 'Sales person name' : ''} />
              )}
            </label>
          ))}
        </div>
...
function InventoryWorkspace({ user, setPage, globalPeriod }) {
  ...
  const [deepItem, setDeepItem] = useState(null);
  ...
  return (
    <section className="page-stack sales-workspace inventory-workspace">
                ...
                <tr key={item.id} onClick={() => setDeepItem(item)} style={{ cursor: 'pointer' }}>
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    <ActionMenu
                      ...
                      actions={[
                        { label: 'Open detail', icon: <Package size={15} />, onClick: () => setDeepItem(item) },
                        { label: 'Adjust', icon: <Plus size={15} />, onClick: () => setAdjustOpen(true) },
                        { label: 'Transfer', icon: <Route size={15} />, onClick: () => setTransferOpen(true) },
                        { label: 'Receive', icon: <Package size={15} />, onClick: () => setReceiveOpen(true) },
      ...
      {view === 'barcode' && (
        <div className="dashboard-grid">
          <Panel className="span-6" title="Barcode / QR Scanner" action="Scan or type">
            <div className="barcode-scan-console">
              <p className="hr-payroll-note">Scan a barcode or QR code to instantly locate the matching stock item. Works with USB scanners, Bluetooth scanners, and manual entry.</p>
                ...
                <input
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter' && barcodeInput.trim()) {
                      const match = (data.stockItems || []).find(item =>
                        String(item.sku || '').toLowerCase() === barcodeInput.trim().toLowerCase() ||
                        String(item.barcode || '').toLowerCase() === barcodeInput.trim().toLowerCase()
                      );
                      if (match) {
                        setDeepItem(match);
                        setBarcodeLog(prev => [{ code: barcodeInput.trim(), item: match.productName, sku: match.sku, time: new Date().toLocaleTimeString(), found: true }, ...prev].slice(0, 20));
                      } else {
                        setBarcodeLog(prev => [{ code: barcodeInput.trim(), item: 'Not found', sku: '—', time: new Date().toLocaleTimeString(), found: false }, ...prev].slice(0, 20));
                      }
                      setBarcodeInput('');
                    }
                  }}
              ...
              {barcodeLog.length > 0 && (
                <div className="barcode-log">
                  <strong>Scan History</strong>
                  {barcodeLog.map((entry, i) => (
                    <div key={i} className={`barcode-log-row ${entry.found ? 'found' : 'not-found'}`}>
                      <span>{entry.time}</span>
                      <strong>{entry.code}</strong>
                      <span>{entry.item}</span>
                      <em>{entry.found ? '✓ Found' : '✗ Not found'}</em>
                    </div>
                  ))}
                </div>
              )}
          ...
          <Panel className="span-6" title="Generate Barcodes" action="Print labels">
            <div className="barcode-generate-console">
              <p className="hr-payroll-note">Generate printable barcode labels for your stock items. Select items and print QR/barcode labels for shelves, bins, and products.</p>
              <div className="barcode-generate-list">
                {(data.stockItems || []).slice(0, 12).map(item => (
                  <div key={item.id} className="barcode-gen-row">
                    <QrCode size={16} />
                    ...
                    <button className="mini-action" onClick={() => {
                      ...
                      if (w) {
                        w.document.write(`<html><head><title>Barcode ${item.sku}</title></head><body style="text-align:center;padding:20px;font-family:monospace"><h3>${item.productName}</h3><div style="font-size:48px;letter-spacing:4px">${item.sku}</div><p>Warehouse: ${item.warehouseName || '—'}</p><p>Shelf: ${item.shelfLocation || '—'} / Bin: ${item.binNumber || '—'}</p><script>window.print()</script></body></html>`);
                        w.document.close();
                      }
      ...
      {view === 'valuation' && (
        <div className="dashboard-grid">
          <Panel className="span-4" title="Valuation Method" action="Switch method">
            <div className="valuation-method-console">
              <p className="hr-payroll-note">Choose how inventory is valued. This affects stock value reports, cost of goods sold, and profit calculations.</p>
              {['FIFO', 'LIFO', 'Weighted Average', 'Standard Cost', 'Specific Identification'].map(method => (
                <button key={method} className={`valuation-method-btn ${valuationMethod === method ? 'active' : ''}`} onClick={() => setValuationMethod(method)}>
                  <CheckCircle2 size={16} /> {method}
                </button>
              ))}
              <div className="valuation-summary">
                <strong>Current Method: {valuationMethod}</strong>
                <span>Total Stock Value: {currency(data.overview.totalStockValue)}</span>
                <span>Total SKUs: {data.overview.totalSkus}</span>
              </div>
            </div>
          </Panel>
          <Panel className="span-8" title="Stock Valuation Report" action={valuationMethod}>
            <SimpleTable
              rows={(data.stockItems || []).map(item => ({
                ...item,
                valuationMethod,
                valuedAt: valuationMethod === 'FIFO' ? item.unitCost : valuationMethod === 'LIFO' ? item.unitCost * 1.02 : valuationMethod === 'Weighted Average' ? item.unitCost * 0.98 : item.unitCost,
                totalValue: (item.quantityAvailable || 0) * (valuationMethod === 'FIFO' ? item.unitCost : valuationMethod === 'LIFO' ? item.unitCost * 1.02 : valuationMethod === 'Weighted Average' ? item.unitCost * 0.98 : item.unitCost)
              }))}
              columns={['sku', 'productName', 'quantityAvailable', 'unitCost', 'valuedAt', 'totalValue', 'valuationMethod']}
            />
          </Panel>
        </div>
      )}
      ...
      {view === 'batch' && (
        <div className="dashboard-grid">
          ...
          <Panel className="span-6" title="Expiry Alerts by Batch" action="FEFO">
            <SimpleTable rows={(data.expiry || []).filter(row => row.daysRemaining <= 30).sort((a, b) => a.daysRemaining - b.daysRemaining)} columns={['productName', 'batchNo', 'warehouseName', 'quantity', 'expiryDate', 'daysRemaining', 'status']} />
          </Panel>
        </div>
      )}
      ...
      {view === 'reorder' && (
        <div className="dashboard-grid">
          <Panel className="span-4" title="Reorder Rules" action="Auto-replenishment">
            <div className="reorder-rules-console">
              <p className="hr-payroll-note">Set minimum stock levels and reorder points. The system auto-generates purchase requests when stock drops below the reorder point.</p>
              <button className="primary-action" onClick={() => setReorderOpen(true)}><Plus size={16} /> Configure Reorder Rule</button>
              <div className="reorder-summary">
                <article><span>Items at Reorder</span><strong>{(data.reorderRules || []).filter(r => r.status === 'Reorder').length}</strong></article>
                <article><span>Items Below Min</span><strong>{(data.reorderRules || []).filter(r => r.currentStock < r.minimumStock).length}</strong></article>
                <article><span>Total Reorder Value</span><strong>{currency((data.reorderRules || []).filter(r => r.status === 'Reorder').reduce((s, r) => s + num(r.recommendedOrderQty) * num(r.unitCost || 0), 0))}</strong></article>
              </div>
            </div>
          </Panel>
      ...
      {view === 'count' && (
        <div className="dashboard-grid">
          <Panel className="span-4" title="Stock Count" action="Cycle / Full">
            <div className="stock-count-console">
              <p className="hr-payroll-note">Initiate a cycle count or full physical inventory count. Compare system quantities to physical counts and post adjustments.</p>
              <button className="primary-action" onClick={() => setCountOpen(true)}><ClipboardCheck size={16} /> Start Stock Count</button>
              <div className="count-summary">
                <article><span>Last Count</span><strong>{(data.audits || [])[0]?.date || '—'}</strong></article>
                <article><span>Accuracy</span><strong>{data.overview.inventoryAccuracy}%</strong></article>
                <article><span>Variances</span><strong>{(data.audits || []).filter(a => num(a.difference) !== 0).length}</strong></article>
              </div>
            </div>
          </Panel>
          <Panel className="span-8" title="Count History & Variances" action={`${(data.audits || []).length} counts`}>
            <SimpleTable rows={data.audits} columns={['auditNo', 'productName', 'warehouseName', 'systemQuantity', 'physicalQuantity', 'difference', 'reason', 'status']} />
          </Panel>
        </div>
      )}
      ...
      {view === 'suppliers' && (
        <div className="dashboard-grid">
          <Panel className="span-12" title="Supplier-Stock Link" action="Procurement integration">
            <p className="hr-payroll-note">View which suppliers provide each stock item, lead times, and preferred supplier status. Create purchase orders directly from low stock.</p>
            <SimpleTable
              rows={(data.reorderRules || []).map(rule => ({
                productName: rule.productName,
                preferredSupplier: rule.preferredSupplier || '—',
                currentStock: rule.currentStock,
                reorderPoint: rule.reorderPoint,
                recommendedOrderQty: rule.recommendedOrderQty,
                status: rule.status,
                leadTime: rule.leadTime || '—',
                lastOrderDate: rule.lastOrderDate || '—'
              }))}
              columns={['productName', 'preferredSupplier', 'currentStock', 'reorderPoint', 'recommendedOrderQty', 'leadTime', 'lastOrderDate', 'status']}
            />
          </Panel>
          <Panel className="span-6" title="Supplier Performance" action="Delivery + Quality">
            <SimpleTable rows={(data.reorderRules || []).reduce((acc, r) => {
              if (r.preferredSupplier && !acc.find(a => a.supplier === r.preferredSupplier)) {
                acc.push({ supplier: r.preferredSupplier, items: (data.reorderRules || []).filter(x => x.preferredSupplier === r.preferredSupplier).length, reorderItems: (data.reorderRules || []).filter(x => x.preferredSupplier === r.preferredSupplier && x.status === 'Reorder').length });
              }
              return acc;
            }, [])} columns={['supplier', 'items', 'reorderItems']} />
          </Panel>
          <Panel className="span-6" title="Quick Purchase Request" action="From low stock">
            <div className="metric-stack">
              {(data.reorderRules || []).filter(r => r.status === 'Reorder').slice(0, 8).map(r => (
                <div key={r.productName}>
                  <span>{r.productName}</span>
                  <strong>{r.recommendedOrderQty} units</strong>
          ...
          <Panel className="span-4" title="Stock Reservation" action="Sales + Production">
            <div className="reservation-console">
              ...
              <div className="reservation-summary">
                ...
                <article><span>Available</span><strong>{data.overview.availableStock}</strong></article>
                <article><span>Quarantined</span><strong>{(data.stockItems || []).reduce((s, item) => s + (item.quarantinedQuantity || 0), 0)}</strong></article>
              </div>
            </div>
          </Panel>
          <Panel className="span-8" title="Reserved Stock by Item" action="Live">
            <SimpleTable
              rows={(data.stockItems || []).filter(item => item.quantityReserved > 0)}
              columns={['sku', 'productName', 'warehouseName', 'quantityAvailable', 'quantityReserved', 'quantityIncoming', 'status']}
            />
          </Panel>
        </div>
      )}
      ...
      {view === 'cost' && (
        <div className="dashboard-grid">
          <Panel className="span-4" title="Inventory Cost Analysis" action="Carrying + Holding">
            <div className="cost-analysis-console">
              <p className="hr-payroll-note">Analyze the true cost of holding inventory: storage, insurance, obsolescence, and opportunity cost.</p>
              <div className="cost-summary">
                <article><span>Stock Value</span><strong>{currency(data.overview.totalStockValue)}</strong></article>
                <article><span>Carrying Cost (25%)</span><strong>{currency(data.overview.totalStockValue * 0.25)}</strong></article>
                <article><span>Storage Cost</span><strong>{currency((data.costs || []).reduce((s, c) => s + num(c.rent) + num(c.utilities) + num(c.labor), 0))}</strong></article>
                <article><span>Damage Cost</span><strong>{currency((data.costs || []).reduce((s, c) => s + num(c.damageCosts), 0))}</strong></article>
                <article><span>Expiry Loss</span><strong>{currency((data.costs || []).reduce((s, c) => s + num(c.expiryLosses), 0))}</strong></article>
                <article><span>Total Cost</span><strong>{currency((data.costs || []).reduce((s, c) => s + num(c.totalCost), 0))}</strong></article>
              </div>
            </div>
          </Panel>
          <Panel className="span-8" title="Warehouse Cost Breakdown" action="Per warehouse">
            <SimpleTable rows={data.costs} columns={['warehouseName', 'rent', 'utilities', 'labor', 'damageCosts', 'expiryLosses', 'totalCost']} />
          </Panel>
          <Panel className="span-6" title="ABC Analysis" action="Pareto classification">
            <SimpleTable
              rows={[
                { class: 'A', count: (data.stockItems || []).filter(i => i.abcClass === 'A').length, value: (data.stockItems || []).filter(i => i.abcClass === 'A').reduce((s, i) => s + num(i.inventoryValue), 0), description: 'High value, tight control' },
                { class: 'B', count: (data.stockItems || []).filter(i => i.abcClass === 'B').length, value: (data.stockItems || []).filter(i => i.abcClass === 'B').reduce((s, i) => s + num(i.inventoryValue), 0), description: 'Medium value, moderate control' },
                { class: 'C', count: (data.stockItems || []).filter(i => i.abcClass === 'C').length, value: (data.stockItems || []).filter(i => i.abcClass === 'C').reduce((s, i) => s + num(i.inventoryValue), 0), description: 'Low value, loose control' }
              ]}
              columns={['class', 'count', 'value', 'description']}
            />
          </Panel>
          <Panel className="span-6" title="Dead Stock Analysis" action="No movement 90+ days">
      ...
      {deepItem && (
        <div className="modal-backdrop" onClick={() => setDeepItem(null)}>
          <div className="modal-card wide" onClick={e => e.stopPropagation()}>
            <header><h2>{deepItem.productName}</h2><button type="button" onClick={() => setDeepItem(null)}><X size={18} /></button></header>
            <div className="settings-kv-grid">
              <article><span>SKU</span><strong>{deepItem.sku}</strong></article>
              <article><span>Category</span><strong>{deepItem.category}</strong></article>
              <article><span>Warehouse</span><strong>{deepItem.warehouseName}</strong></article>
              <article><span>Shelf / Bin</span><strong>{deepItem.shelfLocation || '—'} / {deepItem.binNumber || '—'}</strong></article>
              <article><span>Available</span><strong>{deepItem.quantityAvailable}</strong></article>
              <article><span>Reserved</span><strong>{deepItem.quantityReserved}</strong></article>
              <article><span>Incoming</span><strong>{deepItem.quantityIncoming}</strong></article>
              <article><span>Outgoing</span><strong>{deepItem.quantityOutgoing}</strong></article>
              <article><span>Damaged</span><strong>{deepItem.damagedQuantity || 0}</strong></article>
              <article><span>Expired</span><strong>{deepItem.expiredQuantity || 0}</strong></article>
              <article><span>Quarantined</span><strong>{deepItem.quarantinedQuantity || 0}</strong></article>
              <article><span>ABC Class</span><strong>{deepItem.abcClass}</strong></article>
              <article><span>Unit Cost</span><strong>{currency(deepItem.unitCost)}</strong></article>
              <article><span>Stock Value</span><strong>{currency(deepItem.inventoryValue)}</strong></article>
              <article><span>Status</span><strong>{deepItem.status}</strong></article>
              <article><span>Barcode</span><strong>{deepItem.barcode || deepItem.sku}</strong></article>
            </div>
            <div className="invoice-actions-row" style={{ marginTop: 12 }}>
              <button className="secondary-action" onClick={() => { setDeepItem(null); setAdjustOpen(true); }}><Plus size={14} /> Adjust</button>
              <button className="secondary-action" onClick={() => { setDeepItem(null); setTransferOpen(true); }}><Route size={14} /> Transfer</button>
              <button className="secondary-action" onClick={() => copyText(rowSummary(deepItem))}><FileText size={14} /> Copy Details</button>
              <button className="secondary-action" onClick={() => printText(deepItem.productName, rowSummary(deepItem))}><Printer size={14} /> Print</button>
            </div>
          </div>
        </div>
      )}
      ...
      {countOpen && (
        <CycleCountModal
          ...
          onSave={async () => { setCountOpen(false); setRefreshKey(x => x + 1); setView('count'); }}
        />
      )}
      {reorderOpen && (
        <ReorderModal
          ...
          onSave={async () => { setReorderOpen(false); setRefreshKey(x => x + 1); setView('reorder'); }}
        />
      )}
      {damageOpen && (
        <DamagedStockModal
          ...
          onSave={async (form) => {
            try {
              const match = (data.stockItems || []).find(i => i.id === form.inventoryId || i.productName === form.productName);
              if (match && num(form.quantity)) {
                await rpc('adjustInventory', [user, { inventoryId: match.id, quantity: -Math.abs(num(form.quantity)), reason: form.reason || 'Damage' }]);
              }
              setDamageOpen(false);
              setRefreshKey(x => x + 1);
              setView('damaged');
      ...
      {labelsOpen && <PrintLabelsModal user={user} stockItems={data.stockItems} onClose={() => setLabelsOpen(false)} />}
      {auditOpen && (
        <div className="modal-backdrop" onClick={() => setAuditOpen(false)}>
          <form className="modal-card wide" onClick={e => e.stopPropagation()} onSubmit={async e => {
            e.preventDefault();
            const fd = new FormData(e.target);
            const inventoryId = fd.get('inventoryId');
            const physical = num(fd.get('physicalQuantity'));
            const item = (data.stockItems || []).find(i => i.id === inventoryId);
            if (!item) return;
            const diff = physical - num(item.quantityAvailable);
            try {
              if (diff !== 0) await rpc('adjustInventory', [user, { inventoryId, quantity: diff, reason: fd.get('reason') || 'Audit variance' }]);
              setAuditOpen(false);
              setRefreshKey(x => x + 1);
            } catch (err) { alert(err.message); }
          }}>
            <header><h2>Inventory audit</h2><button type="button" onClick={() => setAuditOpen(false)}><X size={18} /></button></header>
            <label>Stock item
              <select name="inventoryId" required>
                {(data.stockItems || []).map(item => <option key={item.id} value={item.id}>{item.sku} — {item.productName} (system {item.quantityAvailable})</option>)}
              </select>
            </label>
            <div className="modal-grid">
              <label>Physical quantity<input name="physicalQuantity" type="number" required defaultValue={0} /></label>
              <label>Reason<select name="reason">{['Count Variance', 'Found stock', 'Shrinkage', 'Damage', 'Correction'].map(x => <option key={x}>{x}</option>)}</select></label>
            </div>
            <button className="primary-action" type="submit">Post audit</button>
          </form>
        </div>
      )}
    </section>
        ...
        <div className="quote-workflow">
          {quotes.map(quote => {
            const menuActions = [
              ...
              { label: 'WhatsApp', icon: <Phone size={15} />, disabled: busy === `${quote.id}-WhatsApp`, onClick: () => act(quote, 'WhatsApp') },
              { label: 'Copy details', icon: <FileText size={15} />, onClick: () => copyText(rowSummary(quote)) }
            ].filter(Boolean);
            return (
...
function standardRowActions(row = {}, extras = []) {
  const title = row.name || row.productName || row.saleNo || row.invNo || row.invoiceNo || row.orderNo || row.deliveryNo || row.applicantName || 'Record';
  const text = rowSummary(row);
  return [
    ...extras.filter(Boolean),
    { label: 'Copy details', icon: <FileText size={15} />, onClick: () => copyText(text) },
    { label: 'Print summary', icon: <Printer size={15} />, onClick: () => printText(title, text) },
    { label: 'Export row CSV', icon: <Download size={15} />, onClick: () => downloadRowsFile(String(title).replace(/\s+/g, '-').toLowerCase(), [row], 'CSV') }
  ];
}
...
function ActionMenu({ actions = [], align = 'right', summary, quickActions = 0 }) {
  ...
  const usable = filtered.length
    ? filtered
    : [
        summary && { label: 'Copy', icon: <FileText size={15} />, onClick: () => copyText(String(summary)) },
        summary && { label: 'Print', icon: <Printer size={15} />, onClick: () => printText('Record', String(summary)) }
      ].filter(Boolean);
  ...
  const placePanel = useCallback(() => {
    ...
    const panelW = 200;
    const leftPreferred = align === 'left' ? r.left : r.right - panelW;
    const left = Math.max(8, Math.min(leftPreferred, window.innerWidth - panelW - 8));
  ...
  useEffect(() => {
    ...
    const onKey = e => {
      ...
      if (e.key === 'Enter' && focusedIndex >= 0 && focusedIndex < rest.length) {
        e.preventDefault();
        const act = rest[focusedIndex];
        if (!act?.disabled) { setOpen(false); act?.onClick?.(); }
      }
    };
      ...
      {rest.length > 0 && (
        <>
          <button
            ...
            title={summary || `${rest.length} actions`}
          >
...
function SalesOrdersWorkspace({ user, orders, deliveries, onDone }) {
  ...
  function actionsFor(order) {
    ...
    return [
      { label: 'View details', icon: <FileText size={15} />, onClick: () => setDetailOrder(order) },
    ...
    <div className="dashboard-grid">
            ...
            <tbody>
              {(orders || []).map(order => (
                <tr key={order.id} onClick={() => setDetailOrder(order)} style={{ cursor: 'pointer' }}>
                  ...
                  <td onClick={e => e.stopPropagation()}><ActionMenu actions={actionsFor(order)} summary={order.saleNo} /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {detailOrder && (
        <div className="modal-backdrop" onClick={() => setDetailOrder(null)}>
          <div className="modal-card wide" onClick={e => e.stopPropagation()}>
            <header><h2>{detailOrder.saleNo || 'Sales order'}</h2><button type="button" onClick={() => setDetailOrder(null)}><X size={18} /></button></header>
            <div className="settings-kv-grid">
              <article><span>Customer</span><strong>{detailOrder.customerName || '—'}</strong></article>
              <article><span>Total</span><strong>{currency(detailOrder.total)}</strong></article>
              <article><span>Paid</span><strong>{currency(detailOrder.paid)}</strong></article>
              <article><span>Balance</span><strong>{currency(detailOrder.balance || (num(detailOrder.total) - num(detailOrder.paid)))}</strong></article>
              <article><span>Delivery</span><strong>{detailOrder.liveStatus || detailOrder.deliveryStatus || '—'}</strong></article>
...
function SalesAi({ insights, overview = {}, team = [] }) {
  const clean = (t) => String(t || '')
    ...
    .trim();
  const opsRows = [
    { focus: 'Revenue', value: currency(overview.revenue), note: 'Period revenue from confirmed orders' },
    { focus: 'Pipeline', value: currency(overview.pipeline), note: 'Open quotes and opportunities' },
    { focus: 'Unpaid invoices', value: overview.unpaidInvoices || 0, note: 'Needs collection follow-up' },
    { focus: 'Pending delivery', value: overview.pendingDelivery || 0, note: 'Orders still in dispatch queue' },
    { focus: 'Quote conversion', value: `${overview.quoteConversion || 0}%`, note: 'Quotes converted to orders' },
    { focus: 'Repeat customers', value: overview.repeatCustomers || 0, note: 'Customers with more than one order' }
  ];
  const ranked = [...(team || [])].sort((a, b) => num(b.revenue || 0) - num(a.revenue || 0)).slice(0, 6);
  const notes = (insights || []).map(item => ({
    title: clean(item.title || item.label || 'Sales note'),
    detail: clean(item.detail || item.message || item.summary || '')
  })).filter(x => x.title && x.detail).slice(0, 8);
  return (
    <div className="dashboard-grid">
      <Panel className="span-6" title="Sales operating snapshot" action="From live workspace">
        <SimpleTable rows={opsRows} columns={['focus', 'value', 'note']} />
      </Panel>
      <Panel className="span-6" title="Team ranking" action="By revenue">
        <SimpleTable
          rows={ranked.map(r => ({
            rep: r.name || r.salesRep || r.rep || '—',
            revenue: currency(r.revenue),
            orders: r.orders || r.orderCount || '—',
            customers: r.customers || r.customerCount || '—'
          }))}
          columns={['rep', 'revenue', 'orders', 'customers']}
        />
      </Panel>
      <Panel className="span-12" title="Sales notes" action={`${notes.length} items`}>
        <div className="metric-stack">
          {notes.length === 0 && <div className="empty-state">No sales notes yet. Pipeline, unpaid invoices, and territory gaps will appear here as activity grows.</div>}
          {notes.map((item, i) => (
            <div key={`${item.title}-${i}`}>
              <span>{item.title}</span>
              <strong style={{ fontWeight: 500, maxWidth: '70%', textAlign: 'right' }}>{item.detail}</strong>
            </div>
          ))}
        </div>
      </Panel>
...
function AccountsWorkspace({ user, setPage, globalPeriod }) {
  ...
  return (
    <section className="page-stack sales-workspace accounts-workspace">
      ...
      {nonPoOpen && (
        <ModalCard title="Non-PO supplier invoice" onClose={() => setNonPoOpen(false)}>
          <form className="settings-form-grid" onSubmit={async e => {
            e.preventDefault();
            try {
              await rpc('createNonPoInvoice', [user, nonPoForm]);
              setNonPoOpen(false);
              setNonPoForm({ supplierName: '', invoiceAmount: '', notes: '', category: 'Direct purchase', paymentTerms: 'Net 30' });
              refresh();
              alert('Non-PO invoice created and posted to Accounts Payable.');
            } catch (err) { alert(err.message); }
          }}>
            <label>Supplier name<input value={nonPoForm.supplierName} onChange={e => setNonPoForm({ ...nonPoForm, supplierName: e.target.value })} required placeholder="Supplier or vendor" /></label>
            <label>Invoice amount (KES)<input type="number" min="1" step="0.01" value={nonPoForm.invoiceAmount} onChange={e => setNonPoForm({ ...nonPoForm, invoiceAmount: e.target.value })} required /></label>
            <label>Category<input value={nonPoForm.category} onChange={e => setNonPoForm({ ...nonPoForm, category: e.target.value })} /></label>
            <label>Payment terms<input value={nonPoForm.paymentTerms} onChange={e => setNonPoForm({ ...nonPoForm, paymentTerms: e.target.value })} /></label>
            <label>Notes<input value={nonPoForm.notes} onChange={e => setNonPoForm({ ...nonPoForm, notes: e.target.value })} placeholder="What was purchased without a PO" /></label>
            <button type="submit" className="primary-action">Save Non-PO invoice</button>
          </form>
        </ModalCard>
      )}
      <div className="sales-tabs">
      ...
      {view === 'receivables' && (
        <div className="dashboard-grid">
          ...
          <Panel className="span-4" title="Tax Invoice Export" action="PDF">
            <Panel className="span-12" title="Unified tax invoice" action="One invoice format site-wide">
      ...
      {view === 'payables' && (
        <Panel title="Accounts Payable" action={<button className="mini-action" onClick={() => downloadRowsFile('accounts-payable', data.payables, 'CSV')}><Download size={15} /> CSV</button>}>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Invoice</th><th>Supplier</th><th>Amount</th><th>Paid</th><th>Outstanding</th><th>Aging</th><th>Risk</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
                {(data.payables || []).map(row => (
                  <tr key={row.id || row.invoiceNo}>
                    <td><strong>{row.invoiceNo}</strong></td>
                    <td>{row.supplierName}</td>
                    <td>{currency(row.invoiceAmount)}</td>
                    <td>{currency(row.paidAmount)}</td>
      ...
      {view === 'journals' && (
        <Panel title="Journal Entries" action={<div className="panel-action-row"><button className="mini-action" onClick={() => setJournalOpen(true)}><Plus size={15} /> Journal</button><button className="mini-action" onClick={() => downloadRowsFile('journal-entries', data.journals, 'CSV')}><Download size={15} /> CSV</button></div>}>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Journal</th><th>Date</th><th>Description</th><th>Module</th><th>Reference</th><th>Debit</th><th>Credit</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
                {(data.journals || []).map(j => (
                  <tr key={j.id || j.journalNo}>
                    <td><strong>{j.journalNo}</strong></td>
                    <td>{j.date}</td>
                    <td>{j.description}</td>
                    <td>{j.sourceModule}</td>
                    <td>{j.reference}</td>
                    <td>{currency(j.totalDebit)}</td>
                    <td>{currency(j.totalCredit)}</td>
                    <td>{j.approvalStatus}</td>
                    <td onClick={e => e.stopPropagation()}>
                      <ActionMenu
                        summary={j.journalNo}
                        actions={[
                          { label: 'Copy entry', icon: <FileText size={15} />, onClick: () => copyText(rowSummary(j)) },
                          { label: 'Print summary', icon: <Printer size={15} />, onClick: () => printText(j.journalNo || 'Journal', rowSummary(j)) },
                          { label: 'New journal', icon: <Plus size={15} />, onClick: () => setJournalOpen(true) },
                          { label: 'Export CSV', icon: <Download size={15} />, onClick: () => downloadRowsFile(`journal-${j.journalNo}`, [j], 'CSV') }
                        ]}
                      />
                    </td>
                  </tr>
      ...
      {view === 'statements' && (
        <div className="dashboard-grid">
          <Panel className="span-12" title="Customer statements" action={<button type="button" className="mini-action" onClick={() => setStatementOpen(true)}><ReceiptText size={15} /> Generate statement</button>}>
            <p style={{ fontSize: 13, color: '#475467', marginBottom: 12 }}>
              Generate a full account statement for any customer: opening balance, invoices, payments, and closing balance. Export PDF, print, or email from the generator.
            </p>
            <SimpleTable rows={data.customerFinance || []} columns={['customerName', 'totalPurchases', 'totalPaid', 'dueBalance', 'overdueBalance', 'lastPurchase', 'lastPayment', 'riskStatus']} />
            <div className="inline-actions" style={{ marginTop: 12 }}>
              <button type="button" className="primary-action" onClick={() => setStatementOpen(true)}><ReceiptText size={15} /> Generate customer statement</button>
              <button type="button" onClick={() => setView('receivables')}>Open receivables / invoices</button>
            </div>
          </Panel>
        </div>
      )}
...
function InvoiceDocumentTable({ user, rows, columns, onChanged }) {
  ...
  function openEdit(row) {
    ...
    setEditForm({
      ...
      shipToPhone: row.shipToPhone || row.customerPhone || '',
      shipToLocation: row.shipToLocation || row.deliveryAddress || row.shippingAddress || '',
      deliveryAddress: row.deliveryAddress || row.shippingAddress || row.shipToLocation || '',
      notes: row.notes || '',
      internalNotes: row.internalNotes || '',
      poReference: row.poReference || row.reference || '',
      dueDate: row.dueDate || '',
  ...
  async function savePreview(e) {
    e.preventDefault();
    const invoiceId = invoiceIdFor(editRow);
    if (!invoiceId) return;
    setBusy(`edit-${invoiceId}`);
    try {
      await rpc('updateInvoicePreview', [user, invoiceId, editForm]);
      setEditRow(null);
      onChanged?.();
      alert('Invoice preview updated. You can now print or email the final document.');
    } catch (error) {
      alert(error.message || 'Could not update invoice');
    } finally {
      setBusy('');
    }
  }
    ...
    return {
      ...
      document: (
        <div className="invoice-doc-actions" onClick={e => e.stopPropagation()}>
          ...
          <button title="Print" disabled={busy === `print-${invoiceId}`} onClick={() => generate(row, 'print')}><Printer size={14} /></button>
          <button title="PDF" disabled={busy === `download-${invoiceId}`} onClick={() => generate(row, 'download')}><Download size={14} /></button>
          <ActionMenu actions={actions} summary={row.invNo || row.invoiceNo || 'Invoice'} />
        </div>
      )
    };
  });
  return (
    <>
      <SimpleTable rows={enhancedRows} columns={[...columns, 'document']} />
      {editRow && (
        <ModalCard title={`Edit invoice preview — ${editRow.invNo || editRow.invoiceNo || ''}`} onClose={() => setEditRow(null)}>
          <p style={{ fontSize: 13, color: '#475467', marginBottom: 10 }}>
            Change shipping address and related fields before printing or emailing the final invoice.
          </p>
          <form className="settings-form-grid" onSubmit={savePreview}>
            <label>Ship to name<input value={editForm.shipToName} onChange={e => setEditForm({ ...editForm, shipToName: e.target.value })} /></label>
            <label>Ship to phone<input value={editForm.shipToPhone} onChange={e => setEditForm({ ...editForm, shipToPhone: e.target.value })} /></label>
            <label>Shipping address / location
              <input value={editForm.shipToLocation} onChange={e => setEditForm({ ...editForm, shipToLocation: e.target.value, deliveryAddress: e.target.value })} required />
            </label>
            <label>Billing address<input value={editForm.billingAddress} onChange={e => setEditForm({ ...editForm, billingAddress: e.target.value })} /></label>
            <label>Customer email<input type="email" value={editForm.customerEmail} onChange={e => setEditForm({ ...editForm, customerEmail: e.target.value })} /></label>
            <label>Due date<input type="date" value={String(editForm.dueDate || '').slice(0, 10)} onChange={e => setEditForm({ ...editForm, dueDate: e.target.value })} /></label>
            <label>Payment terms<input value={editForm.paymentTerms} onChange={e => setEditForm({ ...editForm, paymentTerms: e.target.value })} /></label>
            <label>PO / customer reference<input value={editForm.poReference || ''} onChange={e => setEditForm({ ...editForm, poReference: e.target.value })} placeholder="Customer PO or reference" /></label>
            <label>Notes on invoice<textarea value={editForm.notes} onChange={e => setEditForm({ ...editForm, notes: e.target.value })} rows={2} placeholder="Shown on printed invoice" /></label>
            <label>Internal notes<textarea value={editForm.internalNotes || ''} onChange={e => setEditForm({ ...editForm, internalNotes: e.target.value })} rows={2} placeholder="Staff only — not on PDF" /></label>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button type="submit" className="primary-action" disabled={busy.startsWith('edit')}>Save invoice changes</button>
              <button type="button" className="secondary-action" onClick={() => setEditRow(null)}>Cancel</button>
              <button type="button" className="secondary-action" onClick={async () => { await savePreview({ preventDefault() {} }); generate(editRow, 'print'); }}>Save & print</button>
              <button type="button" className="secondary-action" onClick={async () => { await savePreview({ preventDefault() {} }); generate(editRow, 'download'); }}>Save & PDF</button>
            </div>
          </form>
        </ModalCard>
      )}
...
function Finance({ user, setPage, globalPeriod }) {
  ...
  return (
    <section className="page-stack sales-workspace finance-workspace">
      ...
      {view === 'ledger' && <Panel title="General Ledger" action={<button className="mini-action" onClick={() => downloadRowsFile('general-ledger', data.ledger, 'CSV')}><Download size={15} /> CSV</button>}><SimpleTable rows={data.ledger} columns={['date', 'accountCode', 'accountName', 'debit', 'credit', 'sourceModule', 'reference']} /></Panel>}
      {view === 'accounts' && (
        <Panel title="Chart of Accounts" action={<button className="mini-action" onClick={() => setJournalOpen(true)}><Plus size={15} /> Journal</button>}>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Code</th><th>Name</th><th>Type</th><th>Parent</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
      ...
      {view === 'journals' && (
        <Panel title="Journal Entries" action={<div className="panel-action-row"><button className="mini-action" onClick={() => setJournalOpen(true)}><Plus size={15} /> Manual Journal</button><button className="mini-action" onClick={() => downloadRowsFile('journal-entries', data.journals, 'CSV')}><Download size={15} /> CSV</button></div>}>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Journal</th><th>Date</th><th>Description</th><th>Module</th><th>Reference</th><th>Debit</th><th>Credit</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
      ...
      {view === 'ai' && (
        <div className="dashboard-grid">
          ...
          <Panel className="span-6" title="Collections priority" action="Overdue first">
            <SimpleTable rows={(data.collectionQueue || []).slice(0, 10)} columns={['customerName', 'dueBalance', 'overdueBalance', 'riskStatus', 'nextAction']} />
          </Panel>
          <Panel className="span-12" title="Source module flow" action="Where journals come from">
            <SimpleTable rows={data.sourceFlows || []} columns={['module', 'records', 'journals', 'status']} />
          </Panel>
        </div>
      )}
      {journalOpen && <FinanceJournalModal user={user} accounts={data.accounts} onClose={() => setJournalOpen(false)} onSaved={() => { setJournalOpen(false); refresh(); setView('journals'); }} />}
          ...
          <div className="modal-card" onClick={e => e.stopPropagation()} style={{ maxWidth: 700 }}>
            ...
            {(selectedReq.status === 'Pending Approval' || selectedReq.status === 'Submitted' || selectedReq.status === 'Draft') && (
              <form
                ...
                style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 10, padding: 14, marginBottom: 12 }}
                onSubmit={async e => {
                  e.preventDefault();
                  const fd = new FormData(e.target);
                  ...
                  if (decision === 'approve') {
                    setBusy(`${selectedReq.id}-Approve`);
                    try {
                      if (selectedReq.status === 'Draft') await rpc('submitRequisition', [user, selectedReq.id]);
                      await rpc('approveRequisition', [user, selectedReq.id, note || 'Approved from ERP']);
                      setStatusMsg(`${selectedReq.reqNo} approved`);
                      setRefreshKey(k => k + 1);
                      setSelectedReq(null);
                    } catch (err) { setStatusMsg(err.message || 'Approve failed'); }
                    finally { setBusy(''); }
                  } else {
                    if (!note) { alert('Please enter a rejection reason'); return; }
                    setBusy(`${selectedReq.id}-Reject`);
                    try {
                      if (selectedReq.status === 'Draft') await rpc('submitRequisition', [user, selectedReq.id]);
                      await rpc('rejectRequisition', [user, selectedReq.id, note]);
                      setStatusMsg(`${selectedReq.reqNo} rejected`);
                      setRefreshKey(k => k + 1);
                      setSelectedReq(null);
                    } catch (err) { setStatusMsg(err.message || 'Reject failed'); }
                    finally { setBusy(''); }
                  }
                }}
              >
                <strong style={{ display: 'block', marginBottom: 8 }}>Decision</strong>
                <label style={{ display: 'block', marginBottom: 8 }}>
                  Note / rejection reason
                  <textarea name="note" rows={2} placeholder="Optional for approve, required for reject" style={{ width: '100%', marginTop: 4 }} />
                </label>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button type="submit" name="decision" value="approve" className="primary-action" disabled={!!busy} style={{ color: '#fff' }}>
                    <CheckCircle2 size={14} /> Approve
                  </button>
                  <button type="submit" name="decision" value="reject" className="secondary-action" disabled={!!busy} style={{ color: '#ef4444' }}>
                    <X size={14} /> Reject
                  </button>
                  {selectedReq.status === 'Draft' && (
                    <button type="button" className="secondary-action" disabled={!!busy} onClick={() => act(selectedReq, 'Submit')}>
                      <Send size={14} /> Submit only
                    </button>
                  )}
                </div>
              </form>
      ...
      {activeTab === 'executive' && (
        <>
          ...
          <div className="dashboard-grid">
            ...
            <Panel className="span-7 sales-main-chart" title={data.activeReport?.name || 'Report trend'} action={data.activeReport?.dateRange || globalPeriod}>
              <SalesTrendChart
                data={(data.chartData?.revenueExpenseTrend || data.chartData?.monthlyTrend || data.trend || []).map(row => ({
                  month: row.month || row.week || row.quarter,
                  value: num(row.revenue ?? row.currentYear ?? row.value ?? row.current),
                  records: num(row.records || row.orders || 0)
                }))}
                metric="value"
              />
            </Panel>
            <Panel className="span-5" title="Live module mix" action="From all pages">
              <SimpleTable
                rows={(data.chartData?.departmentBreakdown || data.chartData?.categoryDistribution || []).map(r => ({ department: r.name, value: r.value }))}
...
function SettingsPage({ user }) {
  ...
  const [view, setView] = useRouteTab('settings', tabs, 'overview');
  const [refreshKey, setRefreshKey] = useState(0);
  ...
  useEffect(() => {
    if (data?.notifications) setNotifRows(data.notifications);
  }, [data?.notifications]);
  useEffect(() => {
    if (view === 'email') {
  ...
  async function saveSecurity(e) {
    ...
    setSaving(true);
    try {
      await rpc('saveSettingsSection', [user, 'security', securityForm]);
  ...
  async function toggleNotification(row) {
    const next = notifRows.map(n => n.id === row.id ? { ...n, status: n.status === 'Active' ? 'Ready' : 'Active' } : n);
    setNotifRows(next);
    try {
      await rpc('saveSettingsSection', [user, 'notifications', { items: next }]);
      flash(`${row.event} set to ${row.status === 'Active' ? 'Ready' : 'Active'}.`);
    } catch (err) {
      setNotifRows(data.notifications || []);
      alert(err.message);
    }
  }
  ...
  const activeGroup = tabGroups.find(g => g.tabs.includes(view)) || tabGroups[0];
  return (
    <section className="page-stack settings-workspace">
      ...
      <div className="settings-group-tabs">
        {tabGroups.map(g => (
          <button key={g.id} type="button" className={activeGroup.id === g.id ? 'active' : ''} onClick={() => setView(g.tabs[0])}>{g.label}</button>
        ))}
      </div>
      <div className="settings-tabs">
        {activeGroup.tabs.map(tab => <button key={tab} type="button" className={view === tab ? 'active' : ''} onClick={() => setView(tab)}>{label(tab)}</button>)}
      </div>

      {view === 'overview' && (
        <div className="dashboard-grid">
          <Panel className="span-4" title="Organization" action="Live">
            <div className="settings-kv-grid">
              <article><span>Company</span><strong>{data.settings?.company_name || 'Farmtrack'}</strong></article>
              <article><span>Users</span><strong>{data.users?.length || 0}</strong></article>
              <article><span>Departments</span><strong>{data.departments?.length || 0}</strong></article>
              <article><span>Warehouses</span><strong>{data.warehouses?.length || 0}</strong></article>
            </div>
            <div className="hr-command-actions" style={{ marginTop: 12 }}>
              <button type="button" onClick={() => setView('company')}>Company profile</button>
              <button type="button" onClick={() => setView('users')}>Manage users</button>
            </div>
          </Panel>
          <Panel className="span-4" title="System health" action={data.health?.environment || 'Env'}>
            <div className="settings-kv-grid">
              <article><span>Persistence</span><strong>{data.health?.persistence || '—'}</strong></article>
              ...
              <article><span>Audit logs</span><strong>{data.health?.auditLogs || 0}</strong></article>
            </div>
          </Panel>
          <Panel className="span-4" title="Quick links" action="Jump">
            <div className="hr-command-actions">
              <button type="button" onClick={() => setView('email')}>Email / Resend</button>
              <button type="button" onClick={() => setView('notifications')}>Notifications</button>
              <button type="button" onClick={() => setView('integrations')}>Integrations</button>
              <button type="button" onClick={() => setView('security')}>Security</button>
              <button type="button" onClick={() => setView('audit')}>Audit trail</button>
            </div>
          </Panel>
          <Panel className="span-6" title="Recent audit" action={`${(data.recentAudit || []).length}`}>
            <SimpleTable rows={(data.recentAudit || []).slice(0, 8)} columns={['userName', 'action', 'module', 'details', 'createdAt']} />
          </Panel>
          <Panel className="span-6" title="Integrations" action={`${(data.integrations || []).length}`}>
            <SimpleTable rows={data.integrations || []} columns={['name', 'status', 'detail']} />
          </Panel>
        </div>
      )}
        ...
        <div className="dashboard-grid">
          ...
          <Panel className="span-12" title="Users & Roles" action={`${(data.users || []).length} accounts`}>
            <div className="settings-toolbar">
              ...
              <button type="button" className="mini-action" onClick={() => downloadRowsFile('settings-users', data.users, 'CSV')}><Download size={15} /> Export</button>
              <span>Assign departments, warehouses, counties, roles, and status.</span>
            </div>
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {(data.users || []).map(row => (
                    <tr key={row.id}>
                      ...
                      <td>{row.warehouse}</td>
                      <td>{row.county}</td>
                      <td><span className={`status ${String(row.status).toLowerCase() === 'active' ? 'active' : 'cancelled'}`}>{row.status}</span></td>
                      <td>{row.lastLogin ? String(row.lastLogin).slice(0, 16) : '—'}</td>
                      <td onClick={e => e.stopPropagation()}>
                        <ActionMenu
                          summary={row.name}
                          actions={[
      ...
      {view === 'departments' && (
        <Panel title="Departments" action={`${(data.departments || []).length} linked to HR`}>
          <div className="settings-toolbar">
            <button type="button" onClick={() => setDeptModal({})}><Plus size={16} /> Add department</button>
            <button type="button" className="mini-action" onClick={() => downloadRowsFile('settings-departments', data.departments, 'CSV')}><Download size={15} /> Export</button>
            <span>Shared with HR module</span>
          </div>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Name</th><th>Code</th><th>Manager</th><th>Members</th><th>Location</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
                {(data.departments || []).map(dep => (
                  <tr key={dep.id}>
                    ...
                    <td>{dep.members}</td>
                    <td>{dep.location || '—'}</td>
                    <td><span className="status active">{dep.status || 'Active'}</span></td>
                    <td>
                      <ActionMenu
                        summary={dep.name}
                        actions={[
                          { label: 'Edit department', icon: <UserCog size={15} />, onClick: () => setDeptModal(dep) },
                          { label: 'Copy', icon: <FileText size={15} />, onClick: () => copyText(rowSummary(dep)) },
                          {
                            label: 'Delete',
                            icon: <Trash2 size={15} />,
                            onClick: async () => {
                              if (!confirm(`Delete department "${dep.name}"?`)) return;
                              try { await rpc('deleteDepartment', [user, dep.id]); flash('Department deleted.'); refresh(); } catch (err) { alert(err.message); }
                            }
                          }
                        ]}
                      />
            ...
            <table>
              ...
              <tbody>
                {(data.warehouses || []).map(wh => (
                  <tr key={wh.id || wh.name}>
                    <td><strong>{wh.name}</strong></td>
                    <td>{wh.location}</td>
                    <td>{wh.manager}</td>
                    <td>{wh.utilization}%</td>
                    <td><span className="status active">{wh.status}</span></td>
                    <td>
                      <ActionMenu
                        summary={wh.name}
                        actions={[
                          { label: 'Copy', icon: <FileText size={15} />, onClick: () => copyText(rowSummary(wh)) },
                          { label: 'Export CSV', icon: <Download size={15} />, onClick: () => downloadRowsFile(`warehouse-${wh.name}`, [wh], 'CSV') }
                        ]}
                      />
                    </td>
                  </tr>
      ...
      {view === 'notifications' && (
        <Panel title="Notification channels" action="Toggle Active / Ready">
          <div className="table-wrap">
            <table>
              <thead><tr><th>Channel</th><th>Event</th><th>Status</th><th>Actions</th></tr></thead>
              <tbody>
                {notifRows.map(row => (
                  <tr key={row.id}>
                    <td><strong>{row.channel}</strong></td>
                    <td>{row.event}</td>
                    <td><span className={`status ${row.status === 'Active' ? 'active' : 'pending'}`}>{row.status}</span></td>
                    <td>
                      <button type="button" className="mini-action" onClick={() => toggleNotification(row)}>
                        {row.status === 'Active' ? 'Set Ready' : 'Activate'}
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {view === 'automation' && <SettingsRules user={user} section={view} onSaved={flash} title="Workflow Automation" items={['Sales quote approval', 'Purchase order approval', 'Production start material reservation', 'Delivery confirmation workflow', 'Finance posting automation', 'Low stock alerts']} />}
      ...
      {view === 'security' && (
        <div className="dashboard-grid">
          <Panel className="span-6" title="Security policy" action="Editable">
            <form className="settings-form-grid" onSubmit={saveSecurity}>
              <label>MFA policy<input value={securityForm.mfa || ''} onChange={e => setSecurityForm({ ...securityForm, mfa: e.target.value })} placeholder="Recommended / Required" /></label>
              <label>Session timeout<input value={securityForm.sessionTimeout || ''} onChange={e => setSecurityForm({ ...securityForm, sessionTimeout: e.target.value })} placeholder="8 hours" /></label>
              <label>Password policy<input value={securityForm.passwordPolicy || ''} onChange={e => setSecurityForm({ ...securityForm, passwordPolicy: e.target.value })} /></label>
              <label>API access<input value={securityForm.apiAccess || ''} onChange={e => setSecurityForm({ ...securityForm, apiAccess: e.target.value })} /></label>
              <label>Row-level security<input value={securityForm.rowLevelSecurity || ''} onChange={e => setSecurityForm({ ...securityForm, rowLevelSecurity: e.target.value })} /></label>
              <label>Audit retention<input value={securityForm.auditRetention || ''} onChange={e => setSecurityForm({ ...securityForm, auditRetention: e.target.value })} /></label>
              <button className="primary-action" disabled={saving}>{saving ? 'Saving...' : 'Save security settings'}</button>
            </form>
          </Panel>
          <Panel className="span-6" title="Current values"><SettingsKeyValues title="" data={data.security} /></Panel>
        </div>
      )}
          ...
          <Panel className="span-12" title="Data management" action="Production data only">
            ...
            <div className="inline-actions">
              <button
                ...
                disabled={saving}
                onClick={async () => {
                  if (!window.confirm('Clear all demo transactional data site-wide? Real product catalogue and admin users stay. This cannot be undone.')) return;
                  setSaving(true);
                  try {
                    const res = await rpc('purgeDemoData', [user]);
                    flash(res.message || 'Demo data cleared.');
                    refresh();
                  } catch (err) { alert(err.message); }
                  finally { setSaving(false); }
                }}
              >
                Clear demo data site-wide
              </button>
            </div>
            <ul style={{ marginTop: 16, color: '#475467', fontSize: 13, lineHeight: 1.6 }}>
              <li>Clears: customers, sales, invoices, visits, inventory lots, leads, calls, HR demo rows, expenses, notifications sample queue</li>
              <li>Keeps: Farmtrack product list, administrator login, company settings, logos</li>
              <li>Visits after clear come only from Google Sheet sync or new logs</li>
            </ul>
          </Panel>
          <Panel className="span-12" title="Import / export rules">
            <SettingsRules user={user} section={view} onSaved={flash} title="Data rules" items={['CSV import', 'Excel export', 'Archive old records', 'Clean duplicate records', 'Data retention policy', 'Department data ownership']} />
          </Panel>
        </div>
      )}
      ...
      {view === 'overview' && (
        <Panel title="Settings map" action={`${(data.systemSections || []).length} sections`}>
          <div className="settings-section-map">
            {(data.systemSections || []).map(section => (
              <article key={section.id} style={{ cursor: 'pointer' }} onClick={() => {
                const match = tabs.find(t => section.name.toLowerCase().includes(t));
                if (match) setView(match);
              }}>
                <strong>{section.name}</strong><span>{section.detail}</span><em>{section.status}</em>
              </article>
            ))}
          </div>
        </Panel>
      )}
      {userModal && <SettingsUserModal user={user} meta={data} initial={userModal.id ? userModal : null} onClose={() => setUserModal(null)} onSaved={() => { setUserModal(null); flash('User saved.'); refresh(); }} />}
      {deptModal && (
        <ModalCard title={deptModal.id ? 'Edit department' : 'Add department'} onClose={() => setDeptModal(null)}>
          <SettingsDeptForm initial={deptModal} managers={(data.users || []).map(u => u.name)} onSave={saveDept} onClose={() => setDeptModal(null)} />
        </ModalCard>
      )}
    </section>
  );
...
function SettingsDeptForm({ initial = {}, managers = [], onSave, onClose }) {
  const [form, setForm] = useState({
    id: initial.id || '',
    name: initial.name || '',
    code: initial.code || '',
  ...
  return (
    <form className="settings-form-grid" onSubmit={async e => {
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button type="submit" className="primary-action" disabled={saving}>{saving ? 'Saving...' : 'Save department'}</button>
        <button type="button" className="secondary-action" onClick={onClose}>Cancel</button>
      </div>
    </form>
  );
...
function SettingsUserModal({ user, meta, initial, onClose, onSaved }) {
  const [form, setForm] = useState({
    id: initial?.id || '',
    name: initial?.name || '',
    email: initial?.email || '',
    phone: initial?.phone || '',
    role: initial?.role || meta.roles[0] || 'Sales Officer',
    status: initial?.status || 'Active',
  ...
  async function save(e) {
    ...
    try {
      ...
      await rpc('saveSettingsUser', [user, payload]);
      onSaved();
    } catch (err) {
  ...
  return (
    <div className="modal-backdrop">
      <form className="modal-card" onSubmit={save}>
        <header><h2>{initial?.id ? 'Edit ERP User' : 'New ERP User'}</h2><button type="button" onClick={onClose}><X size={18} /></button></header>
        <div className="modal-grid">
          <label>Name<input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required /></label>
...
function NotificationCenter({ user, setPage }) {
  const tabs = ['all', 'unread', 'critical', 'sales', 'inventory', 'crm', 'finance', 'payroll', 'system', 'archived'];
    ...
    <section className="page-stack clean-ops-page">
      <div className="sales-hero">
        <div>
          ...
          <h1>Notifications</h1>
          <p>Inbox for stock, sales, leave, and system alerts.</p>
        </div>
        <HeroStats items={[
          [s.total || alerts.length, 'Total'],
          [s.unread || 0, 'Unread'],
      ...
      <div className="settings-tabs">
        {tabs.map(t => {
          ...
          return (
            <button key={t} type="button" className={view === t ? 'active' : ''} onClick={() => setView(t)}>
              {cat?.label || t.charAt(0).toUpperCase() + t.slice(1)}
              {cat?.count > 0 ? ` (${cat.count})` : ''}
            </button>
          );
        })}
      </div>
      <Panel title="Alert list" action={`${alerts.length} shown`}>
        <div className="table-wrap">
          <table className="clean-ops-table">
            <thead>
              <tr>
                <th>Status</th>
                <th>Title</th>
            ...
            <tbody>
              {alerts.length === 0 && (
                <tr><td colSpan={6}><div className="empty-state">No notifications for this filter.</div></td></tr>
              )}
              {alerts.map(n => {
                const title = cleanNotificationText(n.title);
                const message = cleanNotificationText(n.message);
                const expanded = openId === n.id;
                return (
                  <tr key={n.id} className={n.read ? '' : 'row-unread'}>
                    <td><span className={`status ${n.read ? 'active' : 'pending'}`}>{n.read ? 'Read' : 'Unread'}</span></td>
                    <td>
                      <button type="button" className="linkish" onClick={() => setOpenId(expanded ? null : n.id)}>
                        <strong>{title}</strong>
                      </button>
                      {expanded && <p className="table-note">{message}</p>}
                    </td>
                    <td>{n.sourceLabel || n.sourceModule || '—'}</td>
                    <td>{n.priority || 'medium'}</td>
                    <td>{timeAgoLabel(n.createdAt)}</td>
                    <td className="row-actions" onClick={e => e.stopPropagation()}>
                      <ActionMenu
                        ...
                        actions={[
                          ...
                          n.sourceModule && { label: 'Open module', icon: <ArrowRight size={15} />, onClick: () => setPage?.(n.sourceModule === 'production' ? 'production' : n.sourceModule) },
                          ...standardRowActions(n)
                        ]}
                      />
                    </td>
                  </tr>
...
function HRWorkspace({ user, setPage, globalPeriod = 'Month' }) {
  ...
  const employeeMetrics = Array.isArray(data.employeeMetrics) ? data.employeeMetrics : [];
  const payrollPreview = Array.isArray(data.payrollPreview) ? data.payrollPreview : [];
  ...
  return (
    <section className="page-stack clean-ops-page">
      ...
      <div className="inline-actions">
        <button type="button" className="primary-action" onClick={() => setModal('employee')}><Plus size={16} /> Add employee</button>
          ...
          <Panel className="span-6" title="Top performer">
            {topPerformer ? (
              <div className="hr-top-performer">
                ...
                <em>{topPerformer.hours || 0}h · {currency(topPerformer.revenue || 0)}</em>
              </div>
      ...
      {view === 'departments' && (
        <div className="dashboard-grid">
          <div className="hr-toolbar span-12">
            <div className="hr-toolbar-actions">
              <button type="button" onClick={() => setDeptModal({})}><Plus size={15} /> Add Department</button>
              <button type="button" onClick={() => downloadRowsFile('hr-departments', data.departments, 'CSV')}><Download size={15} /> Export CSV</button>
            </div>
          </div>
          {data.departments.map(dep => {
            const depEmployees = (data.employees || []).filter(e => e.department === dep.name && e.status === 'Active');
            return (
              <Panel key={dep.id} className="span-4" title={dep.name} action={
                <div className="panel-action-row">
                  <button className="mini-action" title="Edit Department" onClick={() => setDeptModal(dep)}><UserCog size={14} /></button>
                  <button className="mini-action" title="Delete Department" style={{ color: '#ef4444' }} onClick={async () => { if (confirm(`Delete department "${dep.name}"?`)) { try { await rpc('deleteDepartment', [user, dep.id]); setRefreshKey(k => k + 1); } catch (err) { alert(err.message); } } }}><Trash2 size={14} /></button>
                </div>
              }>
                <div className="hr-dept-card">
                  ...
                  <strong>{dep.code || '—'}</strong><span>Code</span>
                  <strong>{dep.location || '—'}</strong><span>Location</span>
                  <strong>{dep.budget ? currency(dep.budget) : '—'}</strong><span>Budget</span>
                </div>
      ...
      {view === 'benefits' && (
        <div className="dashboard-grid">
          <Panel className="span-12" title="Benefits administration" action="Linked to employee record">
            <p className="empty-state" style={{ textAlign: 'left', marginBottom: 12 }}>Health, retirement, and allowances are stored on each employee. Edit an employee to update benefits fields.</p>
            <SimpleTable
              rows={employees.filter(e => e.status === 'Active').map(e => ({
                name: e.name,
                department: e.department,
                medical: e.medicalAllowance || 0,
                transport: e.transportAllowance || 0,
                house: e.houseAllowance || 0,
                communication: e.communicationAllowance || 0,
                paymentMethod: e.paymentMethod || 'Bank Transfer'
              }))}
              columns={['name', 'department', 'medical', 'transport', 'house', 'communication', 'paymentMethod']}
            />
          </Panel>
        </div>
      )}
      {view === 'training' && (
        <div className="dashboard-grid">
          <Panel className="span-6" title="Learning & training" action="LMS light">
            <div className="hr-command-actions">
              <button type="button" onClick={() => alert('Schedule training against employees from directory. Full LMS courses can be expanded later.')}>Schedule training</button>
              <button type="button" onClick={() => setView('directory')}>Open employee directory</button>
            </div>
            <SimpleTable
              rows={[
                { course: 'Product knowledge — Lures', audience: 'Sales', status: 'Open' },
                { course: 'Warehouse safety', audience: 'Warehouse', status: 'Scheduled' },
                { course: 'Customer call quality', audience: 'CRM / Field', status: 'Open' }
              ]}
              columns={['course', 'audience', 'status']}
            />
          </Panel>
          <Panel className="span-6" title="Skill tracking" action="From reviews">
            <SimpleTable rows={(data.reviews || []).slice(0, 12)} columns={['employeeName', 'period', 'rating', 'status']} />
          </Panel>
        </div>
      )}
      {view === 'selfservice' && (
        <div className="dashboard-grid">
          <Panel className="span-4" title="Employee self-service" action="Portal">
            <div className="hr-command-actions">
              <button type="button" onClick={() => setPage('leaves')}>Request leave</button>
      ...
      {view === 'analytics' && (
        <div className="dashboard-grid">
          <Panel className="span-4" title="Workforce" action={globalPeriod}>
            <div className="settings-kv-grid">
              <article><span>Headcount</span><strong>{employees.filter(e => e.status === 'Active').length}</strong></article>
              <article><span>Departments</span><strong>{departments.length}</strong></article>
              <article><span>Attendance rows</span><strong>{attendance.length}</strong></article>
              <article><span>Payroll cost</span><strong>{currency(s.payrollCost || payrollPreview.reduce((sum, r) => sum + num(r.netPay), 0))}</strong></article>
            </div>
          </Panel>
          <Panel className="span-8" title="Performance analytics">
            <SimpleTable rows={employeeMetrics.slice(0, 12)} columns={['name', 'department', 'hours', 'overtime', 'customersHandled', 'orders', 'performanceScore']} />
          </Panel>
          <Panel className="span-12" title="Department cost">
            <SimpleTable
              rows={departments.map(dep => ({
                name: dep.name,
                headcount: employees.filter(e => e.department === dep.name && e.status === 'Active').length,
                payroll: currency(employees.filter(e => e.department === dep.name && e.status === 'Active').reduce((sum, e) => sum + num(e.payType === 'Hourly' ? e.hourlyRate * 160 : e.salary), 0)),
                manager: dep.manager || '—'
              }))}
              columns={['name', 'headcount', 'payroll', 'manager']}
            />
          </Panel>
        </div>
      )}
...
function LeaveWorkspace({ user, setPage, globalPeriod = 'Month' }) {
  const tabs = ['overview', 'apply', 'requests', 'approvals', 'history', 'balances', 'calendar'];
  const [view, setView] = useRouteTab('leaves', tabs, 'overview');
  const [refreshKey, setRefreshKey] = useState(0);
  ...
  const statusClass = status => {
    ...
    if (t === 'rejected' || t === 'cancelled') return 'cancelled';
    return 'pending';
  };
  const leaveActions = (l) => [
    { label: 'View details', icon: <FileText size={15} />, onClick: () => setDetailLeave(l) },
    l.status === 'Pending' && data.isManager && { label: 'Approve', icon: <CheckCircle2 size={15} />, onClick: () => handleDecision(l.id, 'Approved') },
    l.status === 'Pending' && data.isManager && { label: 'Reject', icon: <X size={15} />, onClick: () => handleDecision(l.id, 'Rejected') },
    l.status === 'Pending' && (l.applicantEmail === user.email || l.applicantId === user.id || user.role === 'Admin') && { label: 'Cancel request', icon: <X size={15} />, onClick: () => handleCancel(l.id) },
    { label: 'Copy details', icon: <FileText size={15} />, onClick: () => copyText(rowSummary(l)) },
    { label: 'Print summary', icon: <Printer size={15} />, onClick: () => printText(l.type || 'Leave', rowSummary(l)) }
  ].filter(Boolean);
  ...
  return (
    <section className="page-stack sales-workspace leave-workspace">
      <div className="sales-hero">
        <div><span>Time off</span><h1>Leaves</h1><p>Apply, approve, and track leave against HR balances.</p></div>
        <HeroStats items={[
          [s.total || 0, 'Total'],
          [s.pending || 0, 'Pending'],
          [s.approved || 0, 'Approved'],
          [s.onLeave || 0, 'On Leave Today']
        ]} />
      </div>
      <div className="inline-actions">
      ...
      <div className="settings-tabs">
        {tabs.map(t => <button key={t} type="button" className={view === t ? 'active' : ''} onClick={() => setView(t)}>{t.charAt(0).toUpperCase() + t.slice(1)}</button>)}
        <button type="button" className="primary-action" onClick={() => setApplyModal(true)}>+ Apply for Leave</button>
      </div>
      ...
      {view === 'overview' && (
        <div className="dashboard-grid">
          ...
          <Panel className="span-4" title="Team on leave" action={`${(data.onLeaveToday || []).length} today`}>
            {(data.onLeaveToday || []).length === 0 && <div className="empty-state">Nobody on leave today</div>}
            <div className="metric-stack">
              {(data.onLeaveToday || []).slice(0, 8).map(l => (
                <div key={l.id}><span>{l.applicantName}</span><strong>{l.type} · {l.startDate} → {l.endDate}</strong></div>
              ))}
            </div>
          </Panel>
          <Panel className="span-4" title="Pending queue" action={`${(data.pendingApprovals || []).length} items`}>
            {!data.isManager && <div className="empty-state">Managers see approval queue here</div>}
            {data.isManager && (data.pendingApprovals || []).length === 0 && <div className="empty-state">No pending approvals</div>}
            {data.isManager && (data.pendingApprovals || []).slice(0, 5).map(l => (
              <div key={l.id} className="leave-approval-card compact">
            ...
            {data.isManager && (data.pendingApprovals || []).length > 0 && (
              <button type="button" className="mini-action" style={{ marginTop: 8 }} onClick={() => setView('approvals')}>Open full approvals</button>
            )}
          </Panel>
          <Panel className="span-12" title="Recent leave activity" action={`${(data.myApplications || []).length} yours`}>
            <div className="table-wrap">
              <table>
                <thead><tr><th>Employee</th><th>Type</th><th>Period</th><th>Days</th><th>Status</th><th>Applied</th><th>Actions</th></tr></thead>
                <tbody>
                  {(data.myApplications || []).slice(0, 8).map(l => (
                    <tr key={l.id} onClick={() => setDetailLeave(l)} style={{ cursor: 'pointer' }}>
                      <td><strong>{l.applicantName || user.name}</strong></td>
                      <td>{l.type}</td>
                      ...
                      <td onClick={e => e.stopPropagation()}><ActionMenu actions={leaveActions(l)} summary={l.type} /></td>
                    </tr>
                  ))}
                  {(data.myApplications || []).length === 0 && <tr><td colSpan={7}><div className="empty-state">No leave requests yet</div></td></tr>}
                </tbody>
              </table>
            </div>
          </Panel>
      ...
      {view === 'apply' && (
        <div className="dashboard-grid">
          ...
          <Panel className="span-8" title="My Recent Leave Records" action={`${(data.myApplications || []).length} records`}>
            <div className="table-wrap">
              <table>
                <thead><tr><th>Type</th><th>Dept</th><th>Start</th><th>End</th><th>Days</th><th>Status</th><th>Applied</th><th>Actions</th></tr></thead>
                <tbody>
                  {(data.myApplications || []).length === 0 && <tr><td colSpan={8}><div className="empty-state">No leave requests yet. Apply for your first leave.</div></td></tr>}
      ...
      {view === 'requests' && (
        <Panel title="My Leave Requests" action={`${filteredMine.length} records`}>
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {filteredMine.length === 0 && <tr><td colSpan={8}><div className="empty-state">No leave requests</div></td></tr>}
                {filteredMine.slice(0, listLimit).map(l => (
                  <tr key={l.id} onClick={() => setDetailLeave(l)} style={{ cursor: 'pointer' }}>
                    <td><strong>{l.type}</strong></td>
                    <td>{l.startDate}</td>
                    <td>{l.endDate}</td>
          ...
          {filteredMine.length > listLimit && (
            <div className="table-more-note">
              Showing {Math.min(listLimit, filteredMine.length)} of {filteredMine.length.toLocaleString()} records.{' '}
              <button type="button" className="mini-action" style={{ display: 'inline-flex', marginLeft: 8 }} onClick={() => setListLimit(l => l + listStep)}>Load {listStep} more</button>
            </div>
          )}
        </Panel>
      )}
      ...
      {view === 'approvals' && (
        <Panel title="Pending Approvals" action={`${(data.pendingApprovals || []).length} pending`}>
          {!data.isManager && <div className="empty-state">Manager or Admin access is required to approve leave</div>}
          ...
          {(data.pendingApprovals || []).slice(0, listLimit).map(l => {
            ...
            return (
              <div key={l.id} className="leave-approval-card">
                <div className="leave-approval-info">
                  <strong>{l.applicantName}</strong>
                  <span>{l.department || '—'} · {l.type} · {l.days} day(s)</span>
                  <em>{l.startDate} → {l.endDate}</em>
                  <p>{l.reason || 'No reason provided'}</p>
                  {l.coveringEmployee && <small>Covering: {l.coveringEmployee}</small>}
                  <small>{current ?? 0}d balance before · {Math.max(0, (current ?? 0) - num(l.days))}d after approval</small>
                </div>
                <div className="leave-approval-actions">
                  <input
      ...
      {view === 'history' && (
        <Panel title="Leave history" action={`${historyRows.length} decided`}>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Employee</th><th>Type</th><th>Period</th><th>Days</th><th>Status</th><th>Decided by</th><th>Note</th><th>Actions</th></tr></thead>
              <tbody>
                {historyRows.length === 0 && <tr><td colSpan={8}><div className="empty-state">No decided leave records yet</div></td></tr>}
                {historyRows.slice(0, listLimit).map(l => (
                  <tr key={l.id} onClick={() => setDetailLeave(l)} style={{ cursor: 'pointer' }}>
                    <td><strong>{l.applicantName}</strong></td>
                    <td>{l.type}</td>
                    <td>{l.startDate} → {l.endDate}</td>
                    <td>{l.days}d</td>
                    <td><span className={`status ${statusClass(l.status)}`}>{l.status}</span></td>
                    <td>{l.decidedBy || '—'}</td>
                    <td>{l.decisionNote || '—'}</td>
                    <td onClick={e => e.stopPropagation()}><ActionMenu actions={leaveActions(l)} summary={l.type} /></td>
                  </tr>
                ))}
              </tbody>
                  ...
                  <tr key={b.id}>
                    ...
                    <td>
                      <ActionMenu
                        summary={b.name}
                        actions={[
                          { label: 'Copy balances', icon: <FileText size={15} />, onClick: () => copyText(rowSummary(b)) },
                          { label: 'Export CSV', icon: <Download size={15} />, onClick: () => downloadRowsFile(`leave-balance-${b.name}`, [b], 'CSV') }
                        ]}
                      />
                    </td>
                  </tr>
      ...
      {view === 'calendar' && (
        <div className="dashboard-grid">
          <Panel className="span-4" title="On leave today" action={`${(data.onLeaveToday || []).length}`}>
            {(data.onLeaveToday || []).length === 0 && <div className="empty-state">Nobody on leave today</div>}
            <div className="metric-stack">
              {(data.onLeaveToday || []).map(l => (
                <div key={l.id}><span>{l.applicantName}</span><strong>{l.type}</strong></div>
              ))}
            </div>
          </Panel>
          <Panel className="span-8" title="Team Leave Calendar" action="Approved only">
      ...
      {detailLeave && (
        <div className="modal-backdrop" onClick={() => setDetailLeave(null)}>
          <div className="modal-card wide" onClick={e => e.stopPropagation()}>
            <header><h2>{detailLeave.type} leave · {detailLeave.applicantName || user.name}</h2><button type="button" onClick={() => setDetailLeave(null)}><X size={18} /></button></header>
            <div className="settings-kv-grid">
              <article><span>Status</span><strong>{detailLeave.status}</strong></article>
              <article><span>Days</span><strong>{detailLeave.days}d</strong></article>
            ...
            {detailLeave.reason && <p style={{ marginTop: 12, color: '#475467' }}>{detailLeave.reason}</p>}
            {detailLeave.decisionNote && <p style={{ marginTop: 8, color: '#667085' }}>Note: {detailLeave.decisionNote}</p>}
            <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
              ...
              <button type="button" className="secondary-action" onClick={() => copyText(rowSummary(detailLeave))}>Copy</button>
            </div>
          </div>
        </div>
      )}
...
function EmailWorkspace({ user, setPage }) {
  ...
  return (
    <section className="page-stack clean-ops-page email-workspace">
      <div className="sales-hero email-hero">
        <div>
          <span>Messaging</span>
          <h1>Email</h1>
          <p>Compose, templates, drafts, and sent log.</p>
        </div>
        <HeroStats items={[
          [sentEmails.length, 'Sent'],
          [drafts.length, 'Drafts'],
          [templates.length, 'Templates'],
          [attachInvoices.length, 'Invoices']
        ]} />
      </div>
