...
function renderMarkdown(text) {
  ...
  // 2. Strip any stray emoji ranges (defensive — backend already removes them)
  t = t.replace(/[\u{1F000}-\u{1FAFF}]/gu, '')
       .replace(/[\u{2600}-\u{27BF}]/gu, '')
       .replace(/[\u{2190}-\u{21FF}]/gu, '')
       .replace(/[\u{2B00}-\u{2BFF}]/gu, '');
  // 3. Remove markdown headers (#, ##, ###) and horizontal rules (---, ___, ***)
  t = t.replace(/^#{1,6}\s+/gm, '');
  t = t.replace(/^\s*([-_*~])\1{2,}\s*$/gm, '');
  // 4. Split into blocks by blank lines so paragraphs render cleanly
  ...
  const htmlBlocks = blocks.map(block => {
    ...
    // Normal paragraph: inline formatting + single newlines become spaces
...
const MODULE_PROMPTS = {
  ...
  production: [
    'How does the production workflow work from materials to finished goods?',
    'Explain formulas and BOM step by step',
    'Show production order status',
    'Which raw materials are low?',
    'How do I request inventory for a production order?',
